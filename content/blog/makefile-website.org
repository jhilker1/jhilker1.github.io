...
While setting up a snippet could be useful for this, hugo has the tools to do it automatically. However, the one thing I haven't had much luck with is using the =read= command in a Makefile. test